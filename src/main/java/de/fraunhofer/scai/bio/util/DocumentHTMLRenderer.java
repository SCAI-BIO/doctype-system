...
public class DocumentHTMLRenderer {
	...
	public static String renderMetaElement(Document document, MetaElement metaElement, List<Annotation> annotations) {
		...
		String date = new SimpleDateFormat("MMMM dd, yyyy").format(metaElement.getBibliographic().getPubDate().getDate().getTime());

		// identifiers
		sb.append(String.format("%s, %s (%s) [lang:%s], %s<br>", 
				metaElement.getBibliographic().getSource().getText(), metaElement.getConcept().getPrefLabel(), date, 
